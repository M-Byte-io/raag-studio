...
// Read the file as a raw buffer, decode it using latin1 to avoid utf8 strictness
let content = fs.readFileSync('js/main.js', 'latin1');

// Fix the corrupted symbols from the original repository:
// In CP1252 / latin1, the em-dash "—" is byte 0x97 (which maps to \x97 in latin1),
// the ellipsis "…" is \x85
// the dot "·" is \xB7
content = content.replace(/\x97/g, '—');
content = content.replace(/\x85/g, '…');
content = content.replace(/\xB7/g, '·');

// Now apply our architectural replacements
...
const oldOnPitchResult = `function _onPitchResult(result) {
...
const newOnPitchResult = `function _onPitchResult(result) {
...
content = content.replace(oldOnPitchResult, newOnPitchResult);
...
// Save strictly as pure utf-8. This permanently cures the bug.
fs.writeFileSync('js/main.js', Buffer.from(content, 'utf8'));
